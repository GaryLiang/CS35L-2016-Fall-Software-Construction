...
int main(int argc, char* argv[])
{
  ...
  fstat(0,&fileStat);
  int init_word_size = 1000;
  int init_ptr_size = 1000000;
  int is_file = 0;
  int is_word_full = 0;
  int is_ptr_full = 0;
  ...
  if(is_file == 0)
      ...
      while(read(0,current_byte,1) > 0)
	{
	  ...
	  if(current_line == vertical_size)
	    {
	        printf("cur == ver\n");
	      if(all = realloc(all,100000*(vertical_size++)))
		{
	  ...
	  if(current_word_index == word_size)
	    {
	       printf("index == words \n");
	      if(all[current_line] = realloc(all[current_line],1000*(word_size++)))
		{
		  if(max_word_size < word_size*1000)
		    max_word_size = word_size*1000;
		  //printf("not enough");
		}
	      else
		{
